...
function selectClassName() {
  return "h-8 w-full min-w-0 rounded-lg border border-input bg-transparent px-2.5 py-1 text-base outline-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 md:text-sm";
}
...
export function QuoteForm({ turnstileSiteKey }: QuoteFormProps) {
  ...
  if (submitState.phase === "success") {
    return (
      <div
        role="status"
        className="border-border bg-brand-soft-green rounded-lg border p-6"
        data-testid="quote-form-success"
      >
        <h2 className="font-heading text-brand-deep-forest text-xl font-semibold">
