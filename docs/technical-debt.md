...
## Homepage motion stability (resolved 2026-07-25 — see ADR-018)

- **RESOLVED — reduced-motion chapters rendered as one stacked, partly hidden scene.** The controller set `data-enhanced="true"` unconditionally, so enhanced-only rules kept every `.solar-chapter` at `position: absolute; inset: 0` and applied `display: none` to `.solar-kicker` / `.solar-lede` / `.solar-text-link`. The reduced-motion overrides meant to undo this existed but were one class short on specificity — `[data-enhanced="true"] .solar-lede` (0,2,0) can never beat `[data-enhanced="true"] :is(.solar-chapter--morning, …) :is(…, .solar-lede, …)` (0,3,0). Fixed by keeping the experience unenhanced under reduced motion and correcting the override specificity. Verified 5/5 repeated runs.
- **RESOLVED — `phase copy windows are exclusive and synchronized` failed intermittently in GitHub Actions.** `data-text-phase` is written synchronously inside the controller's animation frame, but the matching `aria-current="step"` marker is React-rendered and lands on a later commit; the test's fixed 80 ms wait could sample between the two on a loaded runner, and could also sample before the scroll event had scheduled a frame at all. Fixed by publishing `data-solar-ready` and having the test wait for the rendered progress to match the live scroll position instead of a timeout. Verified 10/10 repeated runs plus 43/43 full-suite under `CI=true --workers=1 --retries=0`, and 21/21 under `--workers=2 --repeat-each=3`.
- **Confirmed in CI:** run `30166676853` on `agent/fix-homepage-motion-stability` reported `Lint, typecheck, unit tests, build: success` and `Playwright smoke tests: success` with `43 passed` and no flaky results — the first fully green CI run on this repository since the e2e job started failing.
- **Remaining limitation, recorded honestly:** the phase flake was never reproduced on this Windows workstation; it only ever appeared on GitHub Actions, and a single green CI run cannot prove a timing bug is gone. The fix removes the mechanism that allowed stale sampling, and repeated local runs under CI-like contention plus that CI run are all green. If it ever returns, start from the readiness contract in `src/components/marketing/solar-experience.tsx`, not from the assertions.
