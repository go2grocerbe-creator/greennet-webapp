...
- **Remaining limitation, recorded honestly:** this flake was never reproduced on this Windows workstation — it only ever appeared on GitHub Actions. The fix removes the mechanism that allowed stale sampling, and repeated local runs under CI-like contention are green, but the first CI run on this branch is the real confirmation. If it returns, start from the readiness contract in `src/components/marketing/solar-experience.tsx`, not from the assertions.
