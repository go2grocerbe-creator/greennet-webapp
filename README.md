...
Foundation plus several working features. Structural pages, layouts, auth/middleware scaffolding, typed config, and the Supabase schema/RLS plan exist. The `/contact` page (general contact + quotation request) is fully implemented end to end — form, shared Zod validation, server-side route handler, honeypot + Turnstile + rate-limit checks, Supabase insert, best-effort notification/acknowledgement emails. Admin auth (`/login`, Supabase email/password, owner/editor only) and the admin dashboard (`/admin` — sidebar/topbar shell, quotation summary, quotations list, quotation detail with status update, full Services/Products/Projects CRUD with draft/publish; Site settings is still a placeholder page) are implemented too. Public `/services` now combines a client-confirmed editorial Solar Solutions architecture with the existing published-only CMS catalogue; `/products` also displays only published content. Both reuse the admin data layer directly and RLS remains the public publication gate. Contact links can safely preselect an allow-listed solution interest without trusting arbitrary URL values. All of this still runs against no live Supabase/Resend/Turnstile project (see `.env.example`) — the app fails closed (redirects to `/login`, shows "data unavailable" notices) rather than crashing or faking data. Exact warranties, service areas, certifications, suppliers, monitoring cadence, structural sign-off and maintenance SLAs remain unconfirmed and are not claimed. See `docs/decision-log.md` for the ADR history and `docs/requirements-register.md` §5–6 for what's still MISSING/BLOCKED. Known non-blocking gaps are tracked in `docs/technical-debt.md`.
